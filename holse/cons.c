...
EMCCommand cmds[] = {
	...
	{ "input",
		...
		"\
USAGE: input (0-"MAX_INPUTS_STR") (down|click|dclick|lpress) (toggle|on|off) NUMMASK [timer TIMERNUM]\r\n\
USAGE: input (0-"MAX_INPUTS_STR") (down|click|dclick|lpress) nop [timer TIMERNUM]\r\n\
\r\n\
...
& activate timer TIMERNUM\r\n",
	...
	{ "output",
		...
		"USAGE: output (0-"MAX_INPUTS_STR") (toggle|on|off)\r\n",
		cmdOutput,
		//NULL
	},
	...
	{ "show",
		...
		"\
USAGE: show config\r\n\
USAGE: show (input|timer|output) [0-"MAX_INPUTS_STR"]\r\n\
",
		cmdShow
	},
	...
	{ "timer",
		...
		"\
USAGE: timer (0-"MAX_INPUTS_STR") SECONDS NUMMASK\r\n\
SECOUNDS - Time in Seconds to OFF NUMMASK Output ports\
...
bool cmdInput(char * args) {
	...
	if (!tok) return false;
	int64_t inNum = convertToInt(tok);
...
bool cmdTimer(char * args){
	...
	tok = strtok(NULL, " ");
	if (!tok) return false;
	int64_t secs = convertToInt(tok);
	if (secs < 0) return false;

	//token NUMMASK
	tok = strtok(NULL, " ");
	if (!tok) return false;
	int64_t outs = convertToInt(tok);
	if (outs < 0) return false;
